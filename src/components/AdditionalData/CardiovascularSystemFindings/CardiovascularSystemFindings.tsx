...
const CardiovascularSystemFindings = () => {
  ...
  const handleAdd = async () => {
    try {
      const response = await callApi('cardiovascularSystemFindings', 'post', {
  ...
  const handleUpdate = async () => {
    try {
      ...
      setCardiovascularSystemFindings(
        cardiovascularSystemFindings?.map((item) =>
  ...
  const handleDelete = (id) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await callApi('cardiovascularSystemFindings', 'delete', { id });
          message.success('Deleted successfully');
          setCardiovascularSystemFindings(
            cardiovascularSystemFindings.filter((item) => item._id !== id),
          );
        } catch (error) {
  ...
  const tableData =
    (cardiovascularSystemFindings &&
      cardiovascularSystemFindings?.map((data, index) => ({
        ...data,
        key: index + 1,
        serial: index + 1,
        id: data?._id,
    ...
    <div>
      <div style={{ marginBottom: '16px' }}>
        <Button type="primary" onClick={showModal} icon={<PlusOutlined />}>
          Add Cardiovascular System Findings
        </Button>
      </div>
      ...
      <Modal
        title={
          isEditMode
            ? 'Edit Cardiovascular System Findings'
            : 'Add Cardiovascular System Findings'
        }
...
export default CardiovascularSystemFindings;
